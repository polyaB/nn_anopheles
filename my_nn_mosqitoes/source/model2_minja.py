...
def build_feature_extractor(
    ...
    current = seq_input
    for i in range(11):
        current = conv_block(current, filters = 96,kernel_size=11, dilation_rate=1, pool_size=2, batch_norm=False, activation='elu')
    ...
    current = tf.keras.activations.relu(current)
    final_output = united_emb_layer3(current)
