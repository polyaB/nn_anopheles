...
from basenji.bin.basenji_data_read import read_blacklist
# hic imports
import cooler
...
def main():
    ...
    # seq_hic_raw = genome_hic_cool.matrix(balance=True).fetch("Aalb_2L")
    # print("seq_hic_raw", seq_hic_raw)
    ...
    print(genome_hic_cool.chromnames)
    # print(chr_pre)
    # print(genome_hic_cool.info)
    # for chr in genome_hic_cool.chromnames:
    #   test_hic_raw = genome_hic_cool.matrix(balance=True).fetch(chr)
    #   print(chr, test_hic_raw.shape)
    ...
    for si in range(num_seqs):
        ...
        try:
            ...
            picture_dir = '/mnt/scratch/ws/psbelokopytova/202112281307data_Polya/nn_anopheles/dataset_like_Akita/data/test_new/'
            if mseq.chr == "Aalb_X":
            ...
            if options.as_obsexp:
                ...
                if options.global_obsexp:  # compute global obs/exp
                    ...
                    for i in range(-options.diagonal_offset + 1, options.diagonal_offset): set_diag(seq_hic_obsexp, 1.0,
                                                                                                    i)
                ...
                if options.no_log == False:
                    ...
                    for i in range(-options.diagonal_offset + 1, options.diagonal_offset): set_diag(seq_hic_obsexp, 0,
                                                                                                    i)
                else:
                    if options.clip is not None:
                        seq_hic_obsexp = np.clip(seq_hic_obsexp, 0, options.clip)
                    seq_hic_obsexp = interp_nan(seq_hic_obsexp)
                    for i in range(-options.diagonal_offset + 1, options.diagonal_offset): set_diag(seq_hic_obsexp, 1,
                                                                                                    i)
        ...
        print(np.count_nonzero(seq_hic))

        # write
        # seqs_hic_open['targets'][si, :] = seq_hic.astype('float16')
        targets_mem_map[si] = seq_hic.astype('float16')
