...
def ithwc_MatPlot2HiC(matplot_obj, fname, out_folder, juicer_path=None):
    def ithwc_Pandas2ChrSizes(chrsizes_filename,
        ...
        chrsizes_file = open(chrsizes_filename, 'w')  # creates the file, now the file empty
    ...
    def ithwc_Pandas2Pre(pre_filename, pandas_df):  # This func makes pre-HiC file from the pandas object, control or data
    ...
    ithwc_Pandas2ChrSizes(chromsizes_filename, matplot_obj.data)
    time2 = time.time()
    ...
    time1 = time.time()
    ithwc_Pandas2Pre(pre_data_filename, matplot_obj.data)
    time2 = time.time()
    print('Time: ' + str(round(time2 - time1, 3)) + ' sec\n')
    ...
    time1 = time.time()
    ithwc_Pandas2Pre(pre_control_filename, matplot_obj.control)
    time2 = time.time()
...
def ithwc_from_oe_to_contacts(seq_hic_obsexp, genome_hic_expected_file, interval, seq_len_pool, no_log=False):
    if no_log==False:
...
def ithwc_from_upper_triu(vector_repr, matrix_len, num_diags):
    ...
    for i in range(-num_diags+1,num_diags):
        set_diag(z, np.nan, i)
    return z + z.T
def ithwc_mat_to_pandas_df(mat, binsize, interval):
    starts = []
    ends = []
    values = []
...
def ithwc_plot_juicebox_from_predicted_array(mat, binsize, interval, out_dir,diagonal_offset, use_control=False, **kwargs):
        predicted_data = ithwc_mat_to_pandas_df(mat=mat, binsize=binsize, interval=interval)
        ...
        if not use_control:
            ...
            control_data = ithwc_mat_to_pandas_df(mat=seq_hic_smoothed, binsize=binsize, interval=interval)
        ...
        ithwc_MatPlot2HiC(mp, interval.chr + "_" + str(interval.start) + '_' + str(interval.end) , out_dir+"hic/")

def ithwc_predict_big_region_from_seq(interval,
                                binsize,
                                seq_len,
                                stride,
                                fasta_file,
    ...
    # fname eg is 2L_5000000_8000000
    # fname = interval.chr + "_" + str(interval.start) + '_' + str(interval.end)
    # out_folder = out_dir+"hic/"
    # out_dir = prediction_folder
    # prediction_folder = model_dir
    # model_dir = '/home/konstantin/konstantin/2/nn_anopheles/pleasework/'
    # out_folder = '/home/konstantin/konstantin/2/nn_anopheles/pleasework/hic/'
    fname = interval.chr + "_" + str(interval.start) + '_' + str(interval.end)
    try:
        os.makedirs(prediction_folder+"hic/" + '/' + fname)
        os.makedirs(prediction_folder+"hic/" + '/' + fname + '/pre')
        os.makedirs(prediction_folder+"hic/" + '/' + fname + '/hic')
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise

    def predictor(subinterval):
        ...
        for k_matrix in range(0, k):
            ...
            predicted_mat = ithwc_from_upper_triu(test_pred_from_seq[:, :, 0], target_length_cropped, hic_diags)
        ...
        if returned_to_contacts:
            ...
            mat = ithwc_from_oe_to_contacts(seq_hic_obsexp=mat, genome_hic_expected_file=kwargs['genome_hic_expected_file'],
        ...
        if save_as_hic:
            ...
            ithwc_plot_juicebox_from_predicted_array(mat=mat, binsize=binsize, interval = interval, out_dir=prediction_folder, diagonal_offset=hic_diags, use_control=use_control,
                                               genome_cool_file=kwargs["genome_cool_file"], ghc = cooler.Cooler(kwargs['genome_cool_file']))
    ...
    # TODO is it a problem that cycle uses 'i' and predictor uses 'i'
        ...
        if interval.len // minimal_length == 1:
            ...
            if interval.len % minimal_length == 0:
                ...
                while i != interval.len:
                    predictor(subinterval=Interval(interval.chr, interval.start + i, interval.start + i + minimal_length))
                    i += minimal_length
