...
def plot_juicebox_from_predicted_array(mat, binsize, interval, out_dir,diagonal_offset, use_control=False, **kwargs):
        ...
        print("!!!!!!!!!!!", kwargs["del_start"], kwargs["del_len"])
        #return coordinates

        predicted_data[predicted_data["contact_st"] > int(kwargs["del_start"])] += int(kwargs["del_len"])
        predicted_data[predicted_data["contact_en"] > int(kwargs["del_start"])] += int(kwargs["del_len"])
        if not use_control:
...
def predict_big_region_from_seq(interval,
    ...
    n_end = math.ceil((interval.end - del_len)/binsize)
    ...
    for k_matrix in range(0, k):
        ...
        if seq_start < del_start and seq_end < del_start:
            seq = fasta_open.fetch(chrm, seq_start, seq_end).upper()
        elif seq_start < del_start and seq_end > del_start :
            seq_1 = fasta_open.fetch(chrm, seq_start, del_start).upper()
            seq_2 = fasta_open.fetch(chrm, del_end, del_end+(seq_len-len(seq_1))).upper()
            seq=seq_1+seq_2
            assert len(seq)==seq_len
        elif seq_start > del_start:
            seq = fasta_open.fetch(chrm, seq_start+del_len, seq_end+del_len).upper()
        # print(seq)
    ...
    if save_as_hic:
        ...
        plot_juicebox_from_predicted_array(mat=mat, binsize=binsize, interval = interval, out_dir=prediction_folder, diagonal_offset=hic_diags, use_control=use_control,
                                           genome_cool_file=kwargs["genome_cool_file"], del_len=del_len, del_start=del_start)
